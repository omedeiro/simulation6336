function [x,converged,errf_k,errDeltax_k,relDeltax_k,iterations,X] = NewtonGCR(eval_f,x0,p,u,errf,errDeltax,relDeltax,MaxIter,visualize,FiniteDifference,eval_Jf,tolrGCR,epsMF, gamma, dt)
...
while k<=MaxIter & (errf_k>errf | errDeltax_k>errDeltax | relDeltax_k>relDeltax) % NEWTON LOOP
   f = feval(eval_f,X(:,k),p,u);
   FTRAP = X(:,k) - dt/2*f - gamma;
   if exist('epsMF','var')
      Deltax = tgcr_MatrixFreetrap(eval_f,X(:,k),p,u,-FTRAP,tolrGCR,MaxItersGCR,epsMF,dt); % uses matrix-free
   else
      if FiniteDifference
