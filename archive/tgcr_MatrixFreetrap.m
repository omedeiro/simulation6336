function [x, r_norms] = tgcr_MatrixFree(eval_f,xf,pf,uf,b,tolrGCR,MaxItersGCR,epsMF,dt)
...
k = 0;
while (r_norms(k+1)/r_norms(1) > tolrGCR) && (k <= MaxItersGCR) % GCR LOOP
   k=k+1;
   ...
   %epsilon =                        epsMF/norm(p(:,k)); % already good
   %epsilon =       epsMF*sqrt(1+norm(xf))/norm(p(:,k)); % even better
   epsilon =       epsMF*    (1+norm(xf))/norm(p(:,k)); % this seem best
   %epsilon =     2*epsMF*(1+norm(xf))/norm(p(:,k));     % or this
   %Can also try to pick a value automatically:
   %epsilon =                      pf.dxFD/norm(p(:,k)); % not a bad idea
   %epsilon =   sqrt(eps)*sqrt(1+norm(xf))/norm(p(:,k)); % NITSOL good  
   %epsilon =   sqrt(eps)*(1+norm(xf))/norm(p(:,k));     % this seems BEST
   %epsilon = 2*sqrt(eps)*(1+norm(xf))/norm(p(:,k));     % or this
   
   fepsMF  = feval(eval_f,xf+epsilon*p(:,k),pf,uf);
   f       = feval(eval_f,xf,             pf,uf);
   Ap(:,k) = p(:,k) - dt/2*(fepsMF - f ) / epsilon; 
   ...
   % noticed that if you know A is symmetric
