function F = eval_f_B(X, p, u)
    ...
    y1(p.m1z_int) = p.Bz1;
%     y1(p.mNz_int-(p.Ny+1)*(p.Nx+1)) = reshape(b*a,[],1);
    y1(p.mNzp1_int) = p.Bz1;

    y2(p.m1z_int) = p.Bz2;
    y2(p.mNzp1_int) = p.Bz2;
...
%     figure
%     subplot(1,3,1)
%     scatter3(xx,yy,zz,36,real(y1), 'filled', 'MarkerFaceAlpha', 0.5)
%     axis equal
%     subplot(1,3,2)
%     scatter3(xx,yy,zz,36,real(y2), 'filled', 'MarkerFaceAlpha', 0.5)
%     axis equal
%     subplot(1,3,3)
%     scatter3(xx,yy,zz,36,real(y3), 'filled', 'MarkerFaceAlpha', 0.5)
%     axis equal
%  
...
%             y1 = y1 + sparse(p.m2z_int_cb1, 1, u.Bz*p.hz*p.hx, dim_x, 1);
            
%             y2 = y2 + sparse(p.m2z_int_cb2(12), 1, u.Bz*p.hz*p.hx, dim_x, 1);
%             y1 = y1 + sparse(p.mNz_int_cb2, 1,0, dim_x, 1);
% 
%             y1 = y1 + sparse(p.m2z_int_cb1, 1, u.Bz*p.hz*p.hx, dim_x, 1);
% 
%             y2 = y2 + sparse(p.m2z_int_cb1, 1, 0, dim_x, 1);
%             y2 = y2 + sparse(p.mNz_int_cb1, 1, 0, dim_x, 1);
%     
%             y2 = y2 + sparse(p.m2z_int_cb2, 1, 0, dim_x, 1);
%             y2 = y2 + sparse(p.mNz_int_cb2, 1, 0, dim_x, 1);
%             if p.magBx > 0
